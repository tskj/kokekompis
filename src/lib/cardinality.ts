...
//
//   const cookbook = await db.select().from(cookbooks).where(eq(cookbooks.id, id)).single("lookup");
//   const tag      = await db.select().from(tags).where(...).maybeSingle("tag.lookup"); // null on miss
//   const row      = await db.insert(entries).values({...}).returning().single("insert");
//   const taken    = await db.select({ x: tags.id }).from(tags).where(...).exists();      // boolean
//
