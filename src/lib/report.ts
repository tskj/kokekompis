...
// in kokekompis yet, so report() currently just writes the structured line to the console.

export type ReportPayload = {
  kind: string; // dedup bucket — pick stable, hierarchical names ("auth.sign-in-failed")
  severity?: "warn" | "error";
  message: string;
...
const hookHost = globalThis as typeof globalThis & { __kokekompisOperatorMailer?: OperatorMailer | null };

export function setOperatorMailer(fn: OperatorMailer | null): void {
  hookHost.__kokekompisOperatorMailer = fn;
}

function mailer(): OperatorMailer | null {
  return hookHost.__kokekompisOperatorMailer ?? null;
}
